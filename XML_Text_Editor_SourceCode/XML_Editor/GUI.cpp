...
LRESULT CALLBACK windowprocedure(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
	...
	case WM_COMMAND://lw ay haga etdast
		...
		case GRAPH_REPRESENTATION:
		{
			...
			string temp = g_XML_file_in_one_string;
			Vec_Of_Strings vec = XMLStrToXMLVecOfStr(temp.begin(), temp.end());
			Vec_Of_GraphNodes graphVec = XMLVecOfStrToVecOfGraphNode(vec.begin(), vec.end());
			createGraphVisualization(graphVec);

			string done = "Opening The Graph Visualization...\r\n \r\nOpened successfully.";
			SetWindowText(houtput, (LPCSTR)done.c_str());
			break;
		}
	}
	break;
...
void AddControls(HWND hwnd){
	...
	CreateWindowW(L"Button", L"Graph Visualization", WS_VISIBLE | WS_CHILD, 66, 558, 150, 30, hwnd, (HMENU)GRAPH_REPRESENTATION, NULL, NULL);
